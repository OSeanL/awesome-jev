...
const ja: Messages = {
  ...en,
  seo: {
    title: (n) => `bestjev：${n}件のJevオープンソースプロジェクトとツール`,
    description: (n) => `bestjevで、検証済みのJevオープンソースプロジェクト${n}件を検索。SDK、エージェント、統合、アプリ、ベンチマーク、開発ツールを比較できます。`,
    catalogName: 'bestjev オープンソースプロジェクトカタログ', alternateName: 'bestjev Jevプロジェクトカタログ',
  ...
  hero: { title: '最高のJevプロジェクト', summary: (p, c, l) => `${p}件の検証済みリポジトリを、${c}カテゴリー・${l}言語から検索できます。`, lastUpdated: '最終更新', updatedDate: '2026年9月23日', filterLanguage: '言語で絞り込む', allLanguages: 'すべての言語', languages: '言語', addedToday: '今日追加', addedThisWeek: '今週追加', clearRecency: 'すべての日付を表示', searchPlaceholder: 'プロジェクト、言語、判断を検索', imageAlt: 'heyjunpenn/awesome-jev のGitHubスター数の推移' },
  filterPage: { categoryTitle: (category) => `おすすめJevプロジェクト：${category}`, categorySeoTitle: (category) => `おすすめJevプロジェクト：${category} | bestjev`, categoryDescription: (projects, category) => `bestjevで、${category}のawesomeな検証済みオープンソースJevプロジェクト${projects}件を、GitHubスター、言語、リポジトリ、実装情報から比較できます。`, todayTitle: '今日追加されたJevプロジェクト', todaySeoTitle: '今日の新着Jevオープンソースプロジェクト | bestjev', todayDescription: (projects) => `bestjevに今日追加された${projects}件のオープンソースJevプロジェクトを、検証済みGitHubリポジトリ、言語、スター、実装情報とともに紹介します。`, weekTitle: '今週追加されたJevプロジェクト', weekSeoTitle: '今週の新着Jevオープンソースプロジェクト | bestjev', weekDescription: (projects) => `bestjevに今週追加された${projects}件のオープンソースJevプロジェクトを、検証済みGitHubリポジトリ、言語、スター、実装情報とともに紹介します。` },
